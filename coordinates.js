            ...
            if(geometry.shapeType == 'particles' || geometry.isParticle ||
              ...
              if(geometry?.vertices?.length){
                ...
                if(geometry.isPartitioned){
                  ...
                  px = Math.round((px - geometry.partitions.minX)/geometry.partitionSize)
                  py = Math.round((py - geometry.partitions.minY)/geometry.partitionSize)
                  pz = Math.round((pz - geometry.partitions.minZ)/geometry.partitionSize)
...
const LoadGeometry = async (renderer, geoOptions) => {
  ...
  //if(typeof uvs != 'undefined')  uvs = uvs.map(v=> v -= .001 )
...
const BasicShader = async (renderer, options=[]) => {
                ...
                var uniformOption = {
                  ...
                  vertCode:            ` 
                    ...
                    if(cameraMode == 1.0){
                      ...
                      refractionCamPos = Quat(refractionCamPos, vec3(0.0, 0.0, refractioncOri.z), 1);
                    }
                  ...
                  fragDeclaration:     `
                    ...
                    vec3 Refract(vec3 a, vec3 n, float ip){
                      
                      float d1 = sqrt(a.x * a.x + a.y * a.y + a.z * a.z) + 0.00001;
                      float d2 = sqrt(n.x * n.x + 
                                      n.y * n.y +
                                      n.z * n.z) + 0.00001;
                      ...
                      return vec3(-rx*d1 * ip + a.x*d1 * (1.0 - ip),
                                  -ry*d1 * ip + a.y*d1 * (1.0 - ip),
                                  -rz*d1 * ip + a.z*d1 * (1.0 - ip));
                    }
                  ...
                  fragCode:            `
                    ...
                    if(refractionOmitEquirectangular != 1.0){
                      ...
                      vec3 refractionPos = Refract(testVec, refractionNV, angleOfRefraction);
                      float px = refractionPos.x;
                      float py = refractionPos.y;
                      float pz = refractionPos.z;
                      refractionP1 = atan(px, pz) / M_PI / 2.0;
                      if(cameraMode == 1.0){
  ...
  if(renderer.contextType != '2d') {
    ...
    ret.ConnectGeometry = (geometry, fromNullShader = false) => {
      ...
      var tempCanvases = []
      dataset.optionalUniforms.map((ou, oIdx) => {
        if(ou.canvasTexture){
          tempCanvases.push({
            oid: oIdx,
            ref: ou.canvasTexture
          })
          delete ou.canvasTexture
        }
      })

      var dset = structuredClone(dataset)

      tempCanvases.forEach(tc => {
        dset.optionalUniforms[tc.oid].canvasTexture = tc.ref
      })
